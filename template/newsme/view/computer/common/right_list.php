		...
		<div class="sortby">
			...
			<ul class="sort-list">
				<?php foreach($sortArr as $key=>$value) {?><li<?php echo $param['sort']==$key||(!$param['sort']&&$key==1)?' class="active"':'';?>>
					<a href="?<?php echo http_build_query(array_merge(array_filter($param), ['sort'=>$key]));?>"><?php echo $value;?></a>
					<?php if ($param['sort']==$key||(!$param['sort']&&$key==1)){?><span class="iconfont icon-xuanze"></span><?php }?>
				</li>
				<?php }?>
			</ul>
		</div>
	...
	<?php if (!empty($list)){?><ul class="product-list f0">
		<?php foreach ($list as $value){?><li>
			<a href="<?php echo $value['url'];?>" title="<?php echo $value['name'];?>">
				...
				<p class="name mt12 f16 e3"><?php echo $value['name'];?></p>
