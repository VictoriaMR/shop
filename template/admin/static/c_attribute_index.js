const ATTRIBUTE = {
	init: function() {
		...
		$('.glyphicon-globe').on('click', function(){
			...
			post(URI+'attribute', {opn: 'getAttrLanguage', id: id}, function(data){
		...
		$('#dealbox .save-btn').on('click', function(){
			...
			var obj = $(this);
			obj.button('loading');
			post(URI+'attribute', $('#dealbox form').serializeArray(), function(){
		...
		$('#dealbox-language .save-btn').on('click', function(){
			var obj = $(this);
			obj.button('loading');
			post(URI+'attribute', $('#dealbox-language form').serializeArray(), function(){
				obj.button('reset');
				window.location.reload();
			});
			return false;
		...
		$('.btn.delete').on('click', function(){
			...
			confirm('确定要删除吗?', function(obj){
				obj.button('loading');
				post(URI+'attribute', {opn: 'deleteAttrInfo', id: id}, function(){
					window.location.reload();
				}, function(){
					obj.button('reset');
				});
	...
	loadData: function(id, callback) {
		if (id) {
			post(URI+'attribute', {opn: 'getAttrInfo', id: id}, function(data){
				callback(data);
			});
		} else {
			callback({});
