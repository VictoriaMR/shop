...
class Site extends Base
{
	...
	protected function getSiteLanguage()
	{
		...
		$info = make('app/service/Site')->getLanguage($where);
		$this->success($info ?? [], '');
	}
	...
	protected function getTransfer()
	{
		...
		$result = make('app/service/Translate')->getTranslate($value, $code);
		$this->success($result, '');
	}
	...
	protected function editLanguage()
	{
		...
		if (!empty($language)) {
			$services = make('app/service/Site');
	...
	public function staticCache()
	{
		...
		html()->addJs();
		$files = [];
		$siteList = make('app/service/site/Site')->getListData([], 'name');
	...
	protected function deleteStaticCache()
	{
		...
		if ($name == 'all') {
			$files = [];
			$siteList = make('app/service/Site')->getListData([], 'name');
			foreach ($siteList as $key => $value) {
				$path = ROOT_PATH.$value['name'].DS.'static';
				$this->getFileList($path, $files);
			}
