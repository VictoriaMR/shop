...
class OrderController extends Controller
{
	...
	public function getOrderAddress()
	{
		...
		if (!make('app/service/order/OrderService')->getCountData(['mem_id'=>userId(), 'order_id'=>$orderId])) {
			$this->error('Sorry, we can\' find any order info here, Please try agin.');
		}
		$info = make('app/service/order/AddressService')->loadData(['order_id'=>$orderId, 'type'=>1]);
	...
	public function editOrderAddress()
	{
		...
		$rst = make('app/service/order/AddressService')->updateData(['order_id'=>$orderId, 'type'=>1], $data);
		if ($rst) {
