...
const MEMBERLIST = {
	init: function() {
		...
		$('#dealbox .btn.save').on('click', function(){
			...
			obj.button('loading');
			post('', $(this).parents('form').serializeArray(), function(res){
				showTips(res);
				if (res.code) {
		...
		$('#data-list .switch_botton').on('click', function(){
			...
			post('', {opn:'modify', mem_id: $(this).parents('tr').data('id'), status: status}, function(res) {
				showTips(res);
				if (res.code) {
		...
		$('#data-list .btn.modify').on('click', function(){
			const obj = $(this);
			obj.button('loading');
			post('', {opn:'getInfo', mem_id: obj.parents('tr').data('id')}, function(res) {
				if (res.code) {
