...
const ATTRIBUTE = {
	init: function() {
		...
		$('.btn.modify').on('click', function(){
			...
			post(URI+'desc/descGroup', {opn: 'getDescGroupInfo', id: id}, function(res){
				if (res.code == 200) {
					_this.initData(data);
				} else {
					errorTips(res.msg);
		...
		$('.glyphicon-globe').on('click', function(){
			...
			post(URI+'desc/descGroup', {opn: 'getDescGroupLanguage', id: id}, function(res){
				if (res.code == 200) {
					var data = res.data;
		...
		$('#dealbox .save-btn').on('click', function(){
			...
			var obj = $(this);
			obj.button('loading');
			post(URI+'desc/descGroup', $('#dealbox form').serializeArray(), function(res){
				showTips(res);
				if (res.code == 200) {
		...
		$('#dealbox-language .save-btn').on('click', function(){
			var obj = $(this);
			obj.button('loading');
			post(URI+'desc/descGroup', $('#dealbox-language form').serializeArray(), function(res){
				showTips(res);
				if (res.code == 200) {
					window.location.reload();
				} else {
		...
		$('.btn.delete').on('click', function(){
			...
			confirm('确定要删除吗?', function(obj){
				obj.button('loading');
				post(URI+'desc/descGroup', {opn: 'deleteDescGroupInfo', id: id}, function(res){
					showTips(res);
					if (res.code == 200) {
						window.location.reload();
					} else {
