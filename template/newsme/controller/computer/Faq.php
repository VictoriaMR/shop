...
class Faq extends Base
{
    public function index()
    {
        ...
        if ($fid > 0) {
            $info = service('faq/Faq')->loadData(['faq_id'=>$fid, 'status'=>1], 'faq_id,group_id');
            if (!empty($info)) {
                $where = [];
                if (lanId() != 1) {
                ...
                service('faq/Faq')->where(['faq_id'=>$fid])->increment('visit_total');
                $info = array_merge($info, current(service('faq/FaqLanguage')->getListData($where+['faq_id'=>$fid], 'lan_id,title,content', 0, 0, ['lan_id'=>'DESC'])));
                $info = array_merge($info, current(service('faq/GroupLanguage')->getListData($where+['group_id'=>$info['group_id']], 'lan_id,name', 0, 0, ['lan_id'=>'DESC'])));
                ...
                $faqList = service('faq/Faq')->getListData(['status'=>1, 'group_id'=>$info['group_id']], 'faq_id,group_id');
                if (!empty($faqList)) {
                    $language = service('faq/FaqLanguage')->getListData($where+['faq_id'=>['in', array_column($faqList, 'faq_id')]], 'faq_id,title', 0, 0, ['lan_id'=>'ASC']);
            ...
            $faqList = service('faq/FaqLanguage')->getListData($where);
            if (!empty($faqList)) {
                $faq = service('faq/Faq')->getListData(['faq_id'=>['in', array_column($faqList, 'faq_id')]], 'faq_id,visit_total');
            ...
            $groupList = service('faq/Group')->getListData(['status'=>1], 'group_id,icon');
            if (!empty($groupList)) {
                $language = service('faq/GroupLanguage')->getListData($where+['group_id'=>['in', array_column($groupList, 'group_id')]], 'group_id,name', 0, 0, ['lan_id'=>'ASC']);
                ...
                $faqList = service('faq/Faq')->getListData(['status'=>1], 'faq_id,group_id');
                if (!empty($faqList)) {
                    $language = service('faq/FaqLanguage')->getListData($where+['faq_id'=>['in', array_column($faqList, 'faq_id')]], 'faq_id,title', 0, 0, ['lan_id'=>'ASC']);
                    $language = array_column($language, null, 'faq_id');
                    $temp = [];
