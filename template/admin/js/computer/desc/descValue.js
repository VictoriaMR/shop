...
const ATTRIBUTE = {
	init: function() {
		...
		$('.glyphicon-globe').on('click', function(){
			...
			post(URI+'desc/descValue', {opn: 'getDescvLanguage', id: id}, function(data){
		...
		$('.btn.delete').on('click', function(){
			...
			confirm('确定要删除吗?', function(obj){
				obj.button('loading');
				post(URI+'desc/descValue', {opn: 'deleteDescvInfo', id: id}, function(){
					window.location.reload();
				}, function(){
					obj.button('reset');
				});
	...
	loadData: function(id, callback) {
		if (id) {
			post(URI+'desc/descValue', {opn: 'getDescvInfo', id: id}, function(data){
				callback(data);
			});
		} else {
			callback({});
